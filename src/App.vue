<template>
  ...
  <ul>
    <li v-for="item in provideData.todoList"  :key="item.id">
      ...
      <hr>
      <label for="colors">Onem derecesi seçin:</label>
        <select id="colors" name="color">
          <option value="red">yuksek</option>
          <option value="blue">orta</option>
          <option value="green">dusuk</option>
        </select>
        <hr>
        <input type="checkbox" :id="item.id" v-model="isChecked" @change="disableCheckbox(item.id)" :disabled="isDisabled">
...
const disableCheckbox = (id) => {
  if(id === provideData.value.todoList.id){
    if (isChecked.value) {
